...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${slug} Cluster — Spektt`,
    description: `Join the ${slug} Cluster on Spektt — the creative community platform.`,
    alternates: buildAlternates(locale, `/c/${slug}`),
    openGraph: {
      title: `${slug} Cluster — Spektt`,
      description: `Join the ${slug} Cluster on Spektt — the creative community platform.`,
      url: `https://spektt.com/c/${slug}`,
      siteName: 'Spektt',
 ...
 */
export default async function ClusterFallback({ params }: Props) {
  const { locale: rawLocale, slug } = await params
  const locale = (locales.includes(rawLocale as Locale) ? rawLocale : 'en') as Locale
  ...
  return (
    <div className='min-h-screen bg-warmBlue flex flex-col items-center justify-center px-4 gap-8'>
      ...
      <div className='flex flex-col items-center gap-2 text-center'>
        ...
        <p className='text-textLighter font-regular text-lg'>{dict.fallback.clusterMessage}</p>
      </div>
